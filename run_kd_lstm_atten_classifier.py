...
def prepare_dateset(train_data_path, validation_data_path,test_data_path,vocab):
    ...
    training_review = training.Review[:2]
    ...
    validation_review = validation.Review[:2]
...
def main():
    ...
    LSTM_atten_model =LSTM_atten(vocab_size = vocab_size,hidden_dim = config.HIDDEN_DIM, n_layers =config.N_LAYERS, dropout = 0.3, number_class = args.number_class, bidirectional = True, embedding_dim =100)
