...
def prepare_dateset(train_data_path, validation_data_path, test_data_path, vocab):
    ...
    train_dataset, validation_dataset, testing_dataset = IMDB_kd_indexing(training_texts, training_labels,
                                                                          validation_texts, validation_labels,
                                                                          testing_texts, testing_labels, tokenizers,
                                                                          vocab=vocab)
...
def main():
    ...
    LSTM_atten_model = LSTM_atten(vocab_size=vocab_size, hidden_dim=config.HIDDEN_DIM, n_layers=2, dropout=0.25,
                                  number_class=args.number_class, bidirectional=True, embedding_dim=100)
    ...
    #
    #     train_loss, train_acc = train_kd_fc(training, device, bert_model, LSTM_atten_model, optimizer, criterion,
    #                                         kd_critertion, lr_scheduler)
    #
    ...
    #         torch.save(LSTM_atten_model.state_dict(),
    #                    '/home/dongxx/projects/def-parimala/dongxx/Model_parameter/kd_atten.pt')
    # print("training done")

    print("testing")
    LSTM_atten_model.load_state_dict(
        torch.load('/home/dongxx/projects/def-mercer/dongxx/project/LSTM-baseline/kd_atten_test.pt'))
