...
def main():
    ...
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer,mode='min',factor=0.1,patience=5,verbose=True)
    ...
    for epoch in range(args.num_epochs):
        ...
        if valid_loss < best_loss:
            ...
            torch.save(LSTM_model.state_dict(), config.MODEL_Base_PATH_fk_v2_red)
    print("training done")

    print("testing")

    LSTM_model.load_state_dict(torch.load(config.MODEL_Base_PATH_fk_v2_red))
