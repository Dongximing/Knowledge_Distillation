...
def _setup_kd_datasets(train_text, train_labels, training_logits,validation_text, validation_labels, test_text,test_labels, tokenize,vocab,ngrams=1, include_unk=False):
    ...
    validation_data =_create_data_kd_from_iterator(
        vocab, tokenize,_text_kd_iterator(validation_text, labels=validation_labels,training_logits =None, ngrams=ngrams, yield_label=True), include_unk,
        is_test=False)

    logging.info('Creating testing data')
    test_data= _create_data_kd_from_iterator(
        vocab,tokenize, _text_kd_iterator(test_text, labels=test_labels, training_logits =None,ngrams=ngrams, yield_label=True), include_unk,
        is_test=False)
